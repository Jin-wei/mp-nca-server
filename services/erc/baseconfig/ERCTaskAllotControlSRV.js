...
async function searchT(req, res) {
  try {
    ...
    if (maxLev || maxLev === 0) {
      ...
      let queryStr = 'select * from tbl_erc_taskallotuser t, tbl_common_user u where t.user_id = u.user_id and t.domain_id = ? and t.taskallot_id = ? and t.taskallotuser_level = ?';
      if (doc.customtaskallot_id) {
