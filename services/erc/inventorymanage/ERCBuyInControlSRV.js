...
async function getOtherInOrderMaterielAct(req, res) {
    try {
        ...
        let queryStr = `select s.materiel_id,s.stockapplyitem_id,s.apply_amount,ifnull(s.remain_number, 0) as remain_number,
...
async function getOtherListForMaterielAct(req, res) {
    try {
        ...
        if (doc.warehouse_id != '' && doc.warehouse_zone_id == '') {
            ...
            s.apply_amount,ifnull(s.remain_number, 0) as remain_number,
            b.materiel_code,b.materiel_name,b.materiel_manage,b.materiel_format,b.materiel_unit
            from tbl_erc_stockapplyitem s,tbl_erc_materiel b
...
async function otherInOrderHistoryDetail(req, doc, user) {
    ...
    let queryStr = `select a.bill_code,a.domain_id,a.order_id,a.p_order_id,a.account_operate_amount,
        ...
        from tbl_erc_inventoryaccount a
        left join tbl_erc_materiel b on a.materiel_id = b.materiel_id
        left join tbl_erc_stockapply c on a.p_order_id = c.stockapply_id where true and a.account_operate_type = 3 and a.bill_code = ?`;
    let replacements = [];
    replacements.push(doc.bill_code);
    if (user.domain_id) {
        queryStr += ` and a.domain_id = ?`;
...
async function getQualityCheckOrderAct(req, res) {
    ...
    let queryStr = `select q.qualitycheck_id,cu.name,if(qd.finishStock_number is null, 1, if(qd.finishStock_number < qd.qualified_number, 2, 3)) as order_status,
        if(finishStock_number = 0, 1, if(finishStock_number < qualified_number, 2, 3)) as order_status
        from tbl_erc_qualitycheck q 
        left join tbl_common_user cu on (q.user_id=cu.user_id and cu.state=1) 
        left join (select qualitycheck_id,IFNULL(sum(qualified_number),0) as qualified_number,IFNULL(sum(finishStock_number),0) as finishStock_number from tbl_erc_qualitycheckdetail where state=1 group by qualitycheck_id) qd on (q.qualitycheck_id = qd.qualitycheck_id ) 
        where q.state=1 `;
    if (user.domain_id) {
...
async function getQualityCheckOrderDetailAct(req, res) {
    try{
        ...
        let queryStr = `select qd.qualitycheckdetail_id,qd.order_id,m.materiel_id,m.materiel_code,m.materiel_name,m.materiel_format,m.materiel_unit,qd.qualified_number,qd.finishStock_number,q.supplier_id,s.supplier_name,s.supplier  
            from tbl_erc_qualitycheckdetail qd 
            left join tbl_erc_qualitycheck q on (qd.qualitycheck_id = q.qualitycheck_id and q.state=1) 
            left join tbl_erc_materiel m on (qd.materiel_id = m.materiel_id and m.state=1) 
            left join tbl_erc_supplier s on (q.supplier_id = s.supplier_id and q.state=1) 
            where qd.state=1 and qd.qualified_number<>0 `;
...
async function getQualityCheckListForMaterielAct(req, res) {
    try {
        ...
        queryStr = `select qd.qualitycheckdetail_id,qd.order_id,m.materiel_code,m.materiel_name,
            m.materiel_format,m.materiel_unit,m.materiel_manage,qd.qualified_number,qd.finishStock_number,
            qd.warehouse_id,qd.warehouse_zone_id,0 as stock_operate_amount 
            from tbl_erc_qualitycheckdetail qd 
            left join tbl_erc_qualitycheck q on (qd.qualitycheck_id = q.qualitycheck_id and q.state=1) 
            left join tbl_erc_materiel m on (qd.materiel_id = m.materiel_id and m.state=1) 
            where qd.state=1 and qd.qualitycheck_id=? and qd.qualitycheckdetail_id in (${idArray})`;
        replacements.push(doc.qualitycheck_id);
        if (doc.search_keyword) {
